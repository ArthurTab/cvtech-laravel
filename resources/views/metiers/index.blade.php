    ...
    <div class="flex flex-wrap -mx-3">
        <div class="flex-none w-full max-w-full px-3">
            <div
                ...
                @endif
                <a href="{{route('metiers.create')}}" class="bg-white rounded-lg p-2 text-center text-black mx-6 w-24">Créer</a>
                                ...
                                <tr>
                                    ...
                                    <td class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b border-white/40 whitespace-nowrap shadow-transparent ">
                                        <div class="flex flex-row items-center justify-center space-x-2">
                                            <form action="{{route('metiers.show', $m->id)}}" method="post">
                                                @csrf
                                                @method('GET')
                                                <button
                                            ...
                                            <form action="{{route('metiers.edit', $m->id)}}" method="post">
                                                @csrf
                                                @method('GET')
                                                <button
                                            ...
                                            <form action="{{route('metiers.predelete', $m->id)}}" method="post">
                                                @csrf
                                                @method('GET')
                                                <button
                                                    class="bg-green-900 text-white hover:bg-green-600 p-2 rounded-lg duration-200 hover:scale-110"
